...
import PIL
import time
from dataset.MIMIC_dataset import MIMIC_Dataset, XRayCenterCrop, XRayResizer, MIMIC_Dataset_v2, MIMIC_Dataset_v3, VinBrain_Dataset
...
if __name__ == '__main__':
    ...
    # dataset
    parser.add_argument('--dataset', default='vindr', type=str, help='which dataset', choices=['mimic', 'vindr'])
    ...
    parser.add_argument('--model', default='densenet', type=str, help='pretrained model')
    ...
    parser.add_argument('--data_dir', default='/share/nvmedata/vinbigdata-chest-xray-resized-pt-512x512/rsna_vindr_cnet_csv/', type=str, help='csv file containing the stats')
    ...
    parser.add_argument('--device', default='cuda:0', type=str, help='which device')
    ...
    parser.add_argument('--output_model_path', default='/share/ssddata/MIMIC-densenet-best.pt', type=str, help='model path')
    parser.add_argument('--tag', default='tag', type=str, help='case')
    parser.add_argument('--healthy', default=False, type=bool, help='case')
    ...
    # val_dataset = MIMIC_Dataset_v2(imgpath=im_path, 
    #                               csvpath=args.data_dir + "validate_labels_file_v2.csv",#"mimic-cxr-2.0.0-chexpert.csv.gz",
    #                               metacsvpath=args.data_dir + "validate_main_file_v2.csv",#"mimic-cxr-2.0.0-metadata.csv.gz",
    #                               splitpath = args.data_dir + "mimic-cxr-2.0.0-split.csv.gz", split = 'validate', tag = args.tag,
    #                               transform=None, data_aug=data_aug, unique_patients=False, views=["AP"])

    if args.dataset == 'mimic':
        im_path = "/home/santosh.sanjeev/BGM-CXRRT_analysis/Anees_images/txt_label_generated_epoch_100/"#"/home/santosh.sanjeev/BGM-CXRRT_analysis/image_analysis/CXRBERT_w_clstoken/1_2/gen_1_2/"#"/home/santosh.sanjeev/roentgen/chexray-diffusion/cascaded_gen_1_1_v3/"/home/santosh.sanjeev/roentgen/roentgen_gen_1_1_v2/"#/tmp/san_data/physionet.org/files/mimic-cxr-jpg/2.0.0/files" 
        test_dataset = MIMIC_Dataset_v2(imgpath=im_path, 
                                    csvpath=args.data_dir + "test_labels_file_v2.csv",#"mimic-cxr-2.0.0-chexpert.csv.gz",
                                    metacsvpath=args.data_dir + "test_main_file_v2.csv",#"mimic-cxr-2.0.0-metadata.csv.gz",
                                    splitpath = args.data_dir + "mimic-cxr-2.0.0-split.csv.gz", split = 'test', tag = args.tag,
                                    transform=None, data_aug=data_aug, unique_patients=False, views=["AP"], healthy = args.healthy)
        
    elif args.dataset == 'vindr':
        im_path = '/share/nvmedata/vinbigdata-chest-xray-resized-pt-512x512/test' # None for GT
        test_dataset = VinBrain_Dataset(
            imgpath=im_path, 
    ...
    if "densenet" in args.model:
        ...
        model = models.resnet101(num_classes=test_dataset.labels.shape[1], pretrained=False)
        #patch for single channel
        model.conv1 = torch.nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
        
    elif "resnet50" in args.model:
        model = models.resnet50(num_classes=test_dataset.labels.shape[1], pretrained=False)
        #patch for single channel
        model.conv1 = torch.nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3, bias=False)
        
    elif "shufflenet_v2_x2_0" in args.model:
        model = models.shufflenet_v2_x2_0(num_classes=test_dataset.labels.shape[1], pretrained=False)
        #patch for single channel
        model.conv1[0] = torch.nn.Conv2d(1, 24, kernel_size=3, stride=2, padding=1, bias=False)
    elif "squeezenet1_1" in args.model:
        model = models.squeezenet1_1(num_classes=test_dataset.labels.shape[1], pretrained=False)
        #patch for single channel
    ...
    # print(model)
    # checkpoint_path = '/home/santosh.sanjeev/BGM-CXRRT_analysis/MIMIC_test/MIMIC-densenet-best.pt'
    checkpoint_path = '/share/ssddata/santosh_models/MIMIC-densenet-best.pt'

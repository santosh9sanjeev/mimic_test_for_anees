...
class VinDr_Dataset(Dataset):
    ...
    def __init__(self,
        ...
        if val_set:
            self.rawcsv = self.rawcsv.sample(frac=0.2, random_state=seed)
        else:
            self.rawcsv = self.rawcsv.sample(frac=0.8, random_state=seed)
        
        self.csv = pd.DataFrame(self.rawcsv.groupby("image_id")["class_name"].apply(lambda x: "|".join(np.unique(x))))

        self.paths = self.rawcsv.set_index('image_id')['path'].to_dict() # map image_id to path
        # if imgpath is not None:
        #     self.paths = {k: os.path.join(imgpath, v.split('/')[-1][:-3]) for k, v in self.paths.items()}
    ...
    def __getitem__(self, idx):
        # TODO: load pt from paths
        image_id = self.csv.iloc[idx]["image_id"]
        path = self.paths[image_id]
